...
def transport_cost_model(country_info, bilateral_info, sigma_val, eps_val, error, linear = 'no'):
    ...
    ######## SET COUNTRIES #########
    model.i = Set(initialize = country_info.iso3, doc='Countries iso3 codes')
...
def trade_clearance_calibration(country_info, bilateral_info, sigma_val, eps_val, error, trade_calibration_step1, crop_code, output_file,  count_max = 30, mu_val = 0.01, wtc = 1, wp = 5, wx = 200, max_iter = 500):
    ...
    model1.i = Set(initialize=country_info.iso3, doc='Countries')

    #===================== PARAMETERS=============================================
    #### weighting factors ##
    model1.wtc = Param(initialize= (wtc*factor_tc)/len(country_info.iso3), mutable=True, doc='weight transport cost') ## base = wtc = 1
    model1.wp = Param(initialize= (wp*factor_price)/len(country_info.iso3), mutable=True,doc='weight market supply price') ## wp = 5
    model1.wx = Param(initialize= (wx *factor_trade)/(len(country_info.iso3)**2) , mutable=True,doc='weight trade') ### trade = 200
    ...
    model1.wtc = (scaling_weight) * (wtc*factor_tc)/len(country_info.iso3)
    model1.wp = (scaling_weight) * (wp*factor_price)/(len(country_info.iso3))
    model1.wx = (scaling_weight) *  (wx *factor_trade)/(len(country_info.iso3)**2)
